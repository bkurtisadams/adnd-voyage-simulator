// --- START OF FILE scripts/voyage/passengers.js ---
...
export class PassengerBooking {
    ...
    static async handlePassengerBooking(params) {
        ...
        // 1. Calculate Regular Passengers
        // Formula: 2d4 - 1d4 + Port Size Modifier
        ...
        if (regularPassengers > 0) {
            ...
            // Sum distances of all remaining legs
            for (let j = currentLegIndex; j < routeLegs.length; j++) {
                remainingDistance += (routeLegs[j].distance || 0);
            }
        ...
        // 2. Charter Opportunity (5% chance)
    ...
    static async offerCharterOpportunity(portName, automateTrading) {
        ...
        const segments = Math.ceil(distance / 500);
        const fee = Math.max(100, 40 * segments);
